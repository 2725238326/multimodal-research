...
| 负结果综述稿 | 五轮实验的既有聚合结果整合；数字已对源文件逐项交叉核对；见 `reports/material_constancy_negative_results_synthesis.md` | Completed | 提出稳定性—辨识性权衡：七个干预中无一同时改善翻转率与准确率。可作为投稿基础，但仍需英文重写、权衡关系的受控实验、第二数据集与"专门训练物理辅助模态"分支的对照设计 |
