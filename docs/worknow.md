...
- **产出**：`reports/material_constancy_negative_results_synthesis.md`。
- **统一发现**：稳定性—辨识性权衡。凡是显著降低跨光照翻转率的干预都同时降低材质准确率，反之亦然，七个干预无一同时改善两者。最干净证据是 intrinsic text：两模型翻转率分别降 21.21/30.30 pp（CI 排除零），准确率同时降 23.64/27.88 pp（CI 排除零）；多光照均值把翻转率压到 0.00% 的代价是准确率降 3.03 pp、宏准确率降 5.70 pp。
- **方法论案例**：第四轮开发集 +5.56 pp、方向一致、控制合理，若不坚持预注册 CI 门槛与零重叠外部确认即会得出假阳性；实际外部确认为 −12.50 pp，且打乱（39.58%）与 exposure-only（36.25%）均优于主方法（30.83%）。
- **已更正**：共同区域标记的对比基线是完整场景单框（45.76%/48.79%），不是裁剪 RGB（60.30%/53.03%）；并补记 Qwen 共同框 47.58% 显著低于其原始 RGB 基线 12.73 pp，CI `[-20.30, -5.75]`。
- **下一步（若要投稿）**：英文重写；把权衡关系升级为连续操纵压缩强度的受控实验；补第二数据集支撑外部效度；为"专门训练的物理辅助模态"这一未被排除分支给出对照设计。
